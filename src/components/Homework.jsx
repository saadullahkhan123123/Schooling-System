...
import {
  ...
  Avatar, Stack, Fade, Zoom, CircularProgress
} from '@mui/material';
import {
...
const Homework = () => {
  ...
  const [formData, setFormData] = useState({
    title: '', description: '', subject: '', class: '', dueDate: '', status: 'active',
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const url = editingHomework ? `${API_BASE_URL}/homework/${editingHomework.id}` : `${API_BASE_URL}/homework`;
      const method = editingHomework ? 'PUT' : 'POST';
      const res = await fetch(url, {
        method, headers: getAuthHeaders(), body: JSON.stringify(formData)
      });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/homework/${id}`, { method: 'DELETE', headers: getAuthHeaders() });
    ...
    <Fade in timeout={500}>
      <Box className="p-6">
        <Box className="flex justify-between items-center mb-6">
          <Typography variant="h4" className="font-bold text-[#00335E]">📚 Homework Management</Typography>
        ...
        {loading ? (
          ...
          <Grid container spacing={4}>
            {homeworkData.map(hw => (
              <Grid item xs={12} md={6} lg={4} key={hw.id}>
                <Card className="relative border border-gray-200 hover:shadow-lg transition-shadow">
                  <CardContent>
                    <Box className="flex justify-between items-start mb-2">
                      ...
                      <Box>
                        ...
                        <IconButton onClick={() => handleDelete(hw.id)}>{<DeleteIcon />}</IconButton>
                      </Box>
                    </Box>
        ...
        <Dialog open={openDialog} onClose={handleCloseDialog} maxWidth="md" fullWidth>
          <DialogTitle className="bg-[#00335E] text-white">{editingHomework ? 'Edit Homework' : 'Add Homework'}</DialogTitle>
          <form onSubmit={handleSubmit}>
            <DialogContent className="space-y-4">
              {error && <Box className="bg-red-100 p-2 rounded text-red-600 flex justify-between items-center"><span>{error}</span><IconButton onClick={() => setError(null)}><CloseIcon /></IconButton></Box>}
              {success && <Box className="bg-green-100 p-2 rounded text-green-700 flex justify-between items-center"><span>{success}</span><IconButton onClick={() => setSuccess(null)}><CloseIcon /></IconButton></Box>}
              ...
              <Grid container spacing={2}>
                <Grid item xs={12} md={6}>
                  <FormControl fullWidth>
                    <InputLabel>Class</InputLabel>
                    <Select required value={formData.class} onChange={(e) => setFormData({...formData, class: e.target.value})} label="Class">
                      {classes.map(cls => <MenuItem key={cls} value={cls}>{cls}</MenuItem>)}
                    </Select>
                  </FormControl>
                </Grid>
                <Grid item xs={12} md={6}>
                  <FormControl fullWidth>
                    <InputLabel>Subject</InputLabel>
                    <Select required value={formData.subject} onChange={(e) => setFormData({...formData, subject: e.target.value})} label="Subject">
                      {subjects.map(sub => <MenuItem key={sub} value={sub}>{sub}</MenuItem>)}
                    </Select>
                  </FormControl>
                </Grid>
              ...
              <Grid container spacing={2}>
                <Grid item xs={12} md={6}>
                  <TextField type="date" label="Assigned Date" fullWidth required value={formData.assignedDate} onChange={(e) => setFormData({...formData, assignedDate: e.target.value})} InputLabelProps={{ shrink: true }} />
                </Grid>
                <Grid item xs={12} md={6}>
                  <TextField type="date" label="Due Date" fullWidth required value={formData.dueDate} onChange={(e) => setFormData({...formData, dueDate: e.target.value})} InputLabelProps={{ shrink: true }} />
                </Grid>
              </Grid>
              <FormControl fullWidth>
                <InputLabel>Status</InputLabel>
            ...
            <DialogActions>
              ...
              <Button type="submit" className="bg-[#C99228] text-white hover:bg-[#00335E]">{editingHomework ? 'Update' : 'Add'}</Button>
            </DialogActions>
          </form>
        </Dialog>
      </Box>
